...
from ..cpp_wrappers.cpp_neighbors import radius_neighbors
...
class KPConv(nn.Module):
    ...
    def __repr__(self):
        return 'KPConv(radius: {:.2f}, in_feat: {:d}, out_feat: {:d}, kp_extent: {:f})'.format(self.radius,
                                                                                               self.in_channels,
                                                                                               self.out_channels,
                                                                                               self.KP_extent)
...
class SimpleBlock(nn.Module):
    ...
    def forward(self, queries, supports, q_batches, s_batches, x):
        """
        """

        # Starting radius of convolutions
        r_normal = self.conv_radius

        neighb_inds = self.batch_neighbors(queries, supports, q_batches, s_batches, r_normal)

        # Reduce size of neighbors matrices by eliminating furthest point
        neighb_inds = self.big_neighborhood_filter(neighb_inds)

        x = self.KPConv(queries, supports, neighb_inds, x)
        x = self.norm(x)
        x = self.leaky_relu(x)
        return x

    def batch_neighbors(self, queries, supports, q_batches, s_batches, radius):
        """
        ...
        return radius_neighbors.batch_query(queries, supports, q_batches, s_batches, radius=radius)

    def big_neighborhood_filter(self, neighbors):
        """
        ...
        # if len(self.neighborhood_limit) > 0:
        if self.neighborhood_limit > 0:
            return neighbors[:, :self.neighborhood_limit]
        else:
            return neighbors
